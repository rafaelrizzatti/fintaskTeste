...
from demandas.models import Todo
...
class TodoListCreateAPIViewTestCase(APITestCase):
    ...
    def test_user_todos(self):
        """
        Test to verify user demandas list
        """
...
class TodoDetailAPIViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse("demandas:detail", kwargs={"pk": self.todo.pk})
        self.token = Token.objects.create(user=self.user)
        self.api_authentication()
    ...
    def test_todo_object_bundle(self):
        ...
        todo_serializer_data = DemandaSerializer(instance=self.todo).data
